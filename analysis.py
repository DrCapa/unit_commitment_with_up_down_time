...
def kpi_per_unit(df_data, units, kpis, path_out):
    ...
    dic = {(unit, kpi): s_data_sum[kpi + '_' + unit]
           for unit in units
           for kpi in kpis
           if kpi + '_' + unit in s_data_sum.index}
...
def costs_per_unit(df_data, units, costs,
                  df_chp_old_costs,
                  df_chp_new_costs,
                  df_heat_plant_costs,
                  path_out):
    ...
    for column in df_data.columns:
        if(column.find('gas_') == 0):
            df_temp[column] = df_data[column]*df_data['gas']
        if(column.find('power_') == 0):
            df_temp[column] = -df_data[column]*df_data['spot']
    df_temp.columns = df_temp.columns.str.replace('power_', 'spot_')
    ...
    for unit in units:
        for cost in costs:
            ...
            if((cost == 'on') and
                ...
                'chp_new' in unit):
                df_costs.loc[unit, cost] = df_data_sum[name]*df_chp_new_costs.loc[unit, 'oh']

            if((cost == 'on') and
                (name in df_data_sum) and
                'chp_old' in unit):
                df_costs.loc[unit, cost] = df_data_sum[name]*df_chp_old_costs.loc[unit, 'oh']

            if((cost == 'on') and
                (name in df_data_sum) and
                ('heat_plant' in unit)):
                df_costs.loc[unit, cost] = df_data_sum[name]*df_heat_plant_costs.loc[unit, 'oh']

    for value in df_data_sum.keys():
        if 'StartCost_' in value:
            unit = value.replace('StartCost_', '')
            df_costs.loc[unit, 'start'] = df_data_sum[value]
        
        
    df_costs = df_costs.fillna(0)
    print('total costs: ', df_costs.sum().sum())
